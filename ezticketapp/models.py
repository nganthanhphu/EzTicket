...
class User(UserMixin, BaseModel):
    ...
    avatar = Column(String(255), nullable=True,
                    default="https://res.cloudinary.com/dkzzyue98/image/upload/v1765023207/avatar_ipfsn6.jpg")
    role = Column(Enum(Role), nullable=False, default=Role.CUSTOMER)
    password = Column(String(255), nullable=False)
    active = Column(Boolean, default=True)
    customer_profile = relationship(
        "CustomerProfile", backref="user", uselist=False, lazy=True)


class CustomerProfile(BaseModel):
    user_id = Column(Integer, ForeignKey('user.id'), nullable=False)
    gender = Column(Enum(Gender), nullable=True)
    preferred_event_type_id = Column(
        Integer, ForeignKey('event_type.id'), nullable=True)
...
class Event(BaseModel):
    ...
    event_type_id = Column(Integer, ForeignKey(
    ...
    tickets = relationship("EventTicket", backref="event",
...
class EventTicket(BaseModel):
    ...
    ticket_type_id = Column(Integer, ForeignKey(
...
class OrderItem(BaseModel):
    ...
    event_ticket_id = Column(Integer, ForeignKey(
        'event_ticket.id'), nullable=False)
